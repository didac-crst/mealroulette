...
class HouseholdMembershipService:
    ...
    def ensure_notification_subscription(self, user_id: UUID, household_id: UUID) -> HouseholdNotificationSubscription:
        row = self.db.scalar(
    ...
    def _load_valid_invitation(self, token: str) -> HouseholdInvitation:
        ...
        invitation = self.db.scalar(
            select(HouseholdInvitation).where(HouseholdInvitation.token_hash == token_hash)
        )
    ...
    def accept_invitation_for_user(self, token: str, user: User) -> HouseholdMembership:
        ...
        if existing is not None:
            if existing.active:
                if existing.household_id == invitation.household_id:
